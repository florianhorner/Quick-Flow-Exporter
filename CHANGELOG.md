...
- Fixed rate limit bypass via forged `X-Forwarded-For` header when `TRUST_PROXY=true`: now uses the last (proxy-appended) segment instead of the first (client-supplied, forgeable) segment (`server/proxy.ts`)
