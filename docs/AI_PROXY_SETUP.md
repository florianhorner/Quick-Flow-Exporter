...
`npm start` runs both the Vite UI and the local proxy. Use the provider-specific commands below when you want to override the default provider or run the proxy separately.
...
| `PROXY_PORT`         | `3001`                                      | Proxy port used by Vite when `PORT` is unset |
...
> The Vite dev server proxies `/api` requests to `http://localhost:${PROXY_PORT:-3001}` automatically.
>
